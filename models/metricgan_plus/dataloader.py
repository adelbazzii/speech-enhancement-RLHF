...
class Generator_train_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        noisy_wav, _ = torchaudio.load(
            self.noisy_path + os.path.basename(self.file_list[idx])
        )
...
def create_dataloader(
    ...
    if loader == "G":
        return DataLoader(
            dataset=Generator_train_dataset(filelist, noisy_path, num_target_metric),
        ...
        return DataLoader(
            dataset=Discriminator_train_dataset(filelist, noisy_path, clean_path),
            batch_size=batch_size,
            shuffle=True,
            num_workers=num_workers,
            drop_last=True,
...
def pad_sequence(batch):
    batch = torch.nn.utils.rnn.pad_sequence(batch, padding_value=0.0)
    return batch
...
def collate_fn_G(batch):
    ...
    clean_tensors = pad_sequence(clean_tensors).permute(
        1, 0, 2
    )  # [T, B, F] -> [B, T, F]
    noise_tensors = pad_sequence(noise_tensors).permute(
        1, 0, 2
    )  # [T, B, F] -> [B, T, F]
...
def collate_fn_D(batch):
    ...
    enhanced_tensors = pad_sequence(enhanced_tensors).permute(
        1, 2, 0, 3
    )  # [T, B, 1, F] -> [B, 1, T, F]
    noise_tensors = pad_sequence(noise_tensors).permute(
        1, 2, 0, 3
