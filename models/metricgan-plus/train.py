...
class Trainer:
    def __init__(self, args, data_paths):
        ...
        os.makedirs(
            opj(self.args.output_path, self.args.exp_name, "tmp"), exist_ok=True
        )
        ...
        self.generator_train_paths = list(
            set(self.generator_train_paths) - set(self.generator_valid_paths)
        )
        ...
        with open(opj(self.log_output_path, "log.txt"), "a") as f:
            ...
            f.write(
                f"Train set:{len(self.generator_train_paths)}, Valid set:{len(self.generator_valid_paths)}, Test set:{len(self.generator_test_paths)}\n"
            )
            f.write(
                f"Model parameters:{sum(p.numel() for p in self.G.parameters()) / 10**6:.3f}M\n"
            )

        shutil.copy(
            "train.py", opj(self.args.output_path, self.args.exp_name, "train.py")
        )
    ...
    def load_checkpoint(self, ver="latest"):
        ...
        if ver == "best":
            ...
            print("---load latest model weigths and optimizer---")
    ...
    def train(self):
        ...
        for epoch in np.arange(self.epoch, self.args.epochs + 1):
            ...
            genloader = create_dataloader(
                self.generator_train_paths[0 : round(1 * self.args.num_of_sampling)],
    ...
    def train_one_epoch(self, genloader):
        ...
        if self.epoch >= self.args.skip_val_epoch:
            if self.epoch % self.args.eval_per_epoch == 0:
                self.evaluation(
                    data_list=self.generator_test_paths[
                        0 : self.args.num_of_val_sample
                    ],
                    phase="test",
                )
    ...
    def evaluation(self, data_list, phase="valid"):
            ...
            for i, path in enumerate(tqdm(data_list)):
                ...
                enh_wav = (
                    transform_spec_to_wav(
                        torch.expm1(enh_mag),
                        noise_phase,
                        signal_length=clean_wav.size(1),
                    )
                    .detach()
                    .cpu()
                    .numpy()
        ...
        test_PESQ, test_CSIG, test_CBAK, test_COVL = (
            np.mean(test_PESQ),
            np.mean(test_CSIG),
            np.mean(test_CBAK),
            np.mean(test_COVL),
        ...
        with open(opj(self.log_output_path, "log.txt"), "a") as f:
            f.write(
                f"Epoch:{self.epoch} | Test PESQ:{test_PESQ:.3f} | Test CSIG:{test_CSIG:.3f} | Test CBAK:{test_CBAK:.3f} | Test COVL:{test_COVL:.3f}\n"
            )
    ...
    def get_score(self):
        ...
        with torch.no_grad():
            for path in tqdm(D_paths):
                ...
                enhanced_name = opj(
                    self.train_enhan_path, name + "#" + str(self.epoch) + suffix
                )
                sf.write(enhanced_name, enh_wav, fs)
    ...
    def subtrain_discriminator(self, data_loader, hist=False):
        for i, (enhance_mag, noisy_mag, clean_mag, target) in enumerate(
            tqdm(data_loader)
        ):
            target = target.to(self.device)

            if hist:
                inputs = torch.cat([clean_mag, enhance_mag], dim=1).to(
                    self.device
                )  # (clean, enhanced)
                score = self.D(inputs)
                loss = self.MSELoss(
                    score, target[:, 0]
                )  # target: [C-E score, C-N score, C-C score]
                ...
                for j, target_mag in enumerate([enhance_mag, noisy_mag, clean_mag]):
                    inputs = torch.cat([clean_mag, target_mag], dim=1).to(
                        self.device
                    )  # (clean, enhanced), (clean, noisy), (clean, clean)
                    score = self.D(inputs)
    ...
    def train_discriminator(self):
        ...
        disc_loader_hist = create_dataloader(
            train_concat_set,
            self.train_noisy_path,
            clean_path=self.train_clean_path,
            loader="D",
