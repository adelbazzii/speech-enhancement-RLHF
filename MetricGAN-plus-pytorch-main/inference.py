...
def main():
    ...
    parser.add_argument(
        "--weight_path", type=str, default="results/exp1/model", help="Model path"
    )
    ...
    parser.add_argument(
        "--base_path", type=str, default="data/VCTK_DEMAND", help="Data base path"
    )
...
class Tester:
    def __init__(self, args, data_paths):
        ...
        print(
            f"Total parameters:{sum(p.numel() for p in self.G.parameters()) / 10**6:.3f}M"
        )
    ...
    def load_checkpoint(self):
        checkpoint = torch.load(
            opj(self.model_weight_path, self.args.weight_file),
    ...
    def evaluation(self, data_list, phase="valid"):
        ...
        with torch.no_grad():
            for i, path in enumerate(tqdm(data_list)):
                ...
                enhanced_name = opj(
                    self.args.weight_path, self.args.save_wavs_path, f"{name}{suffix}"
                )
        ...
        print(
            f"Test PESQ:{test_PESQ:.3f} | Test CSIG:{test_CSIG:.3f} | Test CBAK:{test_CBAK:.3f} | Test COVL:{test_COVL:.3f}"
        )
