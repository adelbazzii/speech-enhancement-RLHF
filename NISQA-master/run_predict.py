...
parser.add_argument(
    "--csv_file", type=str, help="file name of csv (must be in current working folder)"
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="number of workers for pytorchs dataloader",
)
parser.add_argument("--bs", type=int, default=1, help="batch size for predicting")
parser.add_argument(
    "--ms_channel", type=int, help="audio channel in case of stereo file"
)
...
if args["mode"] == "predict_file":
    ...
    if args["csv_deg"] is None:
        raise ValueError(
            "--csv_deg argument with csv column name of the filenames needed"
        )
    if args["data_dir"] is None:
        args["data_dir"] = ""
else:
